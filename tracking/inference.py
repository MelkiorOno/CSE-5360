...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        poses = util.Counter()
        for old_position, old_probility in self.beliefs.items():
            for new_position, new_probility in self.getPositionDistribution(self.setGhostPosition(gameState, old_position)).items():
                poses[new_position] += old_probility * new_probility
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        self.particles = [self.legalPositions[k % len(self.legalPositions)]
                          for k in xrange(self.numParticles)]
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        init = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(init)
        self.particles = [init[k % len(init)] for k in xrange(self.numParticles)]
    ...
    def observeState(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        for k, remove in enumerate(noisyDistances):
            if remove is None:
                self.particles = [self.getParticleWithGhostInJail(p, k)
                                  for p in self.particles]
        beliefs = util.Counter()
        for p in self.particles:
            prob = 1
            for k, ghost, model, remove in \
                    zip(xrange(len(p)), p, emissionModels, noisyDistances):
                if remove is not None:
                    prob *= model[util.manhattanDistance(pacmanPosition, ghost)]
                else:
                    if self.getJailPosition(k) != ghost:
                        prob = 0
            beliefs[p] += prob
        if any(i != 0 for i in beliefs.values()):
            self.particles = [util.sample(beliefs) for i in self.particles]
        else:
            self.initializeParticles()
            for index, remove in enumerate(noisyDistances):
                if remove is None:
